...
import com.imkola.client.chat.presenter.IGroupMsgPresenter;
import com.imkola.client.chat.view.IGroupMsgView;
...
public class GroupMsgPresenter implements IGroupMsgPresenter {
    ...
    public void syncMessageStatus(List<Message> messages){
        try{
            ...
            if (msgIds.size() >0) {
                String curSiteIdentity = currentSite.getSiteIdentity();
                IMClient.getInstance(new com.imkola.client.socket.SiteAddress(curSiteIdentity)).syncMessageStatus(msgIds, CoreProto.MsgType.GROUP_TEXT_VALUE);
                return;
            }
        } catch (Exception e) {
