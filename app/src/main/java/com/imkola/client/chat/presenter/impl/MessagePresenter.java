package com.imkola.client.chat.presenter.impl;
...
import com.imkola.client.plugin.task.GetMsgPluginListTask;
import com.imkola.client.site.presenter.impl.SitePresenter;
import com.imkola.client.util.MsgUtils;
import com.imkola.client.util.NetUtils;
import com.imkola.client.util.data.StringUtils;
import com.imkola.client.util.file.ImageRoateUtil;
    ...
    protected void sendMsgByIMManger(Message message) {
        ...
        if (isNet) {
            try {
                ...
                IMClient.getInstance(new com.imkola.client.socket.SiteAddress(curSiteIdentity)).sendMessage(message);
            } catch (Exception e) {
                ZalyLogUtils.getInstance().errorToInfo(TAG, e.getMessage());
            }
        }
    ...
    public void syncMessageStatus(List<Message> messages) {
        try {
            ...
            if (msgIds.size() > 0) {
                ...
                String curSiteIdentity = currentSite.getSiteIdentity();
                IMClient.getInstance(new com.imkola.client.socket.SiteAddress(curSiteIdentity)).syncMessageStatus(msgIds, CoreProto.MsgType.TEXT_VALUE);
                return;
            }
