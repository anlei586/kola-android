...
public class SPUtils {
    ...
    public void putKey(@NonNull final String key, @NonNull final String value) {
        String base64Value = Base64.encodeToString(value.getBytes(), Base64.NO_WRAP);
        put(key, base64Value, false);
    }

    public String getKey(@NonNull final String key) {
        try {
            String base64Value = getString(key);
            return new String(Base64.decode(base64Value, Base64.NO_WRAP));
        } catch (Exception e) {
            Logger.e(e);
            return null;
        }
