package com.imkola.client.util.file; /**
...
//        }
    ...
    public static Bitmap blur(Context context, Bitmap sentBitmap, int radius) {
        ...
        if (radius < 0) {
            radius = 0;
            if (DEBUG) {
                Logger.w(TAG, "radius must be 0 < r < 25 , forcing radius=0");
            }
        } else if (radius > 25) {
            radius = 25;
            if (DEBUG) {
                Logger.w(TAG, "radius must be 0 < r < 25 , forcing radius=25");
            }
        }
    ...
    /**
     * 将YUV格式的图片的源数据从横屏px2dip模式转为竖屏模式，注意：将源图片的宽高互换一下就是新图片的宽高
     *
